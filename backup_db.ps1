...
Write-Host "Backup de BD SIGEL (localhost:5433)..." -ForegroundColor Cyan
...
docker run --rm -e PGPASSWORD=$env:PGPASSWORD -v "${PWD}:/out" postgres:16-alpine sh -c "pg_dump -h host.docker.internal -p 5433 -U postgres -d SIGEL -F c -f /out/$archivo"
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "Error al crear el backup. ¿PostgreSQL está corriendo en localhost:5433?" -ForegroundColor Red
    exit 1
}
