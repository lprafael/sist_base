...
async def main():
    ...
    async with engine.begin() as conn:
        print(f"1. Creando Dojos para Torneo ID: {target_torneo_id}")
        dojo_map = {}
        ...
        print("3. Limpiando jugadores anteriores...")
        await conn.execute(text("""
        ...
        print("4. Insertando 30 atletas en torneos.tournament_players...")
        random.seed(42)
        for i in range(1, 31):
            ...
            print(f" [{i}/30] {nombre_completo} ({dojo_nombre}) - {categoria_nombre} - {peso_verificado}kg [{estado}]")

    print("\nVerificando en base de datos...")
    async with engine.connect() as conn:
        res_count = await conn.execute(text("""
        ...
        print(f"TOTAL CONFIRMADO EN BD: {cnt} atletas en torneos.tournament_players")
