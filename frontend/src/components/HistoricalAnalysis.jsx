...
const HistoricalAnalysis = ({ user }) => {
    ...
    const [distId, setDistId] = useState(user.distrito_id || 27); // San Lorenzo por defecto
    ...
    useEffect(() => {
        fetchData();
        if (isAdmin && dptoId) fetchDistritos(dptoId);
            ...
            <header className="analysis-header">
                ...
                <div className="analysis-controls">
                    <div className="control-group">
                        ...
                        <select
                            value={dptoId}
                            onChange={e => { setDptoId(parseInt(e.target.value)); setDistId(null); }}
                            disabled={!isAdmin}
                        >
                    ...
                    <div className="control-group">
                        ...
                        >
                            {!isAdmin && <option value={distId}>Heredado de Perfil</option>}
                            {isAdmin && catalog.dists.map(d => <option key={d.id} value={d.id}>{d.nombre}</option>)}
                        </select>
                    </div>
