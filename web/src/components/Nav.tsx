...
export default function Nav({ scrolled }: NavProps) {
  ...
  return (
    <>
          ...
          {session ? (
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              {(session.role === 'organizador' || session.is_organizador) && (
                <a href="/admin-futbol/campeonatos" className="btn btn-outline btn-sm nav-desktop-action">
                  Panel Torneos
                </a>
              )}
              {(session.role === 'academia' || session.academia_id) && (
                <a href="/academia-panel" className="btn btn-outline btn-sm nav-desktop-action">
                  Panel Academia
                </a>
              )}
              ...
              {session.role === 'admin' && (
                <a href="/admin" className="btn btn-outline btn-sm nav-desktop-action">
                  Consola Clubes
                </a>
              )}
      ...
      {mobileMenuOpen && (
        <div className="nav-mobile-drawer open" onClick={() => setMobileMenuOpen(false)}>
          <a href="/buscar" className="nav-mobile-link" style={{ color: '#4ade80' }}>
            <span>🗺️</span> Buscar en Mapa
          </a>
          <a href="#canchas" className="nav-mobile-link">
            <span>🏟️</span> Lista Canchas
          </a>
          <a href="/torneos" className="nav-mobile-link">
            <span>🏆</span> Torneos
          </a>
          ...
          {session && (
            <div style={{ marginTop: '12px', borderTop: '1px solid rgba(255,255,255,0.1)', paddingTop: '12px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {(session.role === 'organizador' || session.is_organizador) && (
                <a href="/admin-futbol/campeonatos" className="nav-mobile-link">
                  <span>⚙️</span> Panel Torneos
                </a>
              )}
              {(session.role === 'academia' || session.academia_id) && (
                <a href="/academia-panel" className="nav-mobile-link">
                  <span>🎓</span> Panel Academia
                </a>
              )}
              {(session.role === 'complejo' || session.complejo_id) && (
              ...
              {session.role === 'admin' && (
                <a href="/admin" className="nav-mobile-link">
                  <span>🛡️</span> Consola Clubes
                </a>
              )}
            </div>
          )}
