...
async def run_final_repair():
    ...
    print("Iniciando reparación final de electoral.padrones...")
    start_time = time.time()
    
    async with async_session() as s:
        # 1. Eliminar la restricción de FK que causa el error
        print("Eliminando restricción padrones_local_id_fkey...")
        try:
            await s.execute(text("ALTER TABLE electoral.padrones DROP CONSTRAINT IF EXISTS padrones_local_id_fkey"))
            await s.commit()
        except Exception as e:
            print(f"Nota: No se pudo eliminar la restricción (quizás ya no existe): {e}")
            await s.rollback()

        # 2. Actualizar Elección 1 (ANR 2026)
