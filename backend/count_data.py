...
with engine.connect() as conn:
    print("Checking cartografia.barrios data...")
    ...
    print(f"Total barrios: {total}")
    print(f"Barrios with population: {with_pop}")
    print(f"Barrios linked to RefDistrito: {with_ref}")
    
    if with_pop > 0:
        sample = conn.execute(text("SELECT \"BARLO_DESC\", poblacion_total FROM cartografia.barrios WHERE poblacion_total > 0 LIMIT 5")).fetchall()
